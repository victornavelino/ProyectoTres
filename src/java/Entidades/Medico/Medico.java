...
@NamedQueries({
    ...
    @NamedQuery(name = "Medico.buscarMedicosDeudoresEntre", query = "SELECT med FROM Medico med WHERE med.tipoSocio.id = 1 EXCEPT SELECT m FROM Medico m where m.id IN (SELECT p.medico.id FROM Pago p where p.anio>=:anio AND p.mes>=:mes) EXCEPT SELECT m FROM Medico m where m.id IN (SELECT p.medico.id FROM Pago p where p.anio>=:anio2 AND p.mes>=:mes2) ")
})
