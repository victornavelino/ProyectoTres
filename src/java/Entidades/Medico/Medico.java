...
@NamedQueries({
    ...
    @NamedQuery(name = "Medico.buscarMedicosDeudores", query = "SELECT med FROM Medico med WHERE med.tipoSocio.id = 1 EXCEPT SELECT m FROM Medico m where m.id IN (SELECT p.medico.id FROM Pago p where p.anio>=:anio AND p.mes>:mes)"),
    @NamedQuery(name = "Medico.buscarMedicosDeudoresEntre", query = "SELECT med FROM Medico med WHERE med.tipoSocio.id = 1  AND med.pagos IS NOT EMPTY "
            + "AND med.id NOT IN (SELECT p.medico.id FROM Pago p where p.anio>=:anio AND p.mes>:mes) "
            + "AND med.id IN (SELECT p.medico.id FROM Pago p where p.anio>=:anio2 AND p.mes>=:mes2)"
    )
})
